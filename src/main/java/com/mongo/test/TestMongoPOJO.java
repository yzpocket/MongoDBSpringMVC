...
//static import문을 통해서 클래스명을 생략하고 사용해보자. 
//eq(), gt(), lt(), gte()... 등
...
public class TestMongoPOJO {
	...
	public void insertMany() {
		...
		List<PostVO> postsArr=Arrays.asList(
				new PostVO(null,"Scott","추운날씨","2022-12-14"),
				new PostVO(null,"Davi","추운날씨","2022-12-14"),
				new PostVO(null,"Scole","추운날씨","2022-12-14")
				);
...
//		System.out.println(arr);
	...
	public void findByAuthor() {
		...
		//PostVO vo1=cursor.first(); //1개만 가져오고싶을때 아래 반복문은 모두. 
	...
	public void deleteOne() {
		...
		System.out.println(res.getDeletedCount()+"개 도큐먼트 삭제 됨");
	}//----------------------------------
	
	public void updateOne() {
		System.out.println("수정할 글의 작성자명 입력: ");
		String name=sc.nextLine();
		System.out.println("수정할 글 제목: ");
		String title=sc.nextLine();
		System.out.println("수정할 글 작성일 [yyyy-MM-dd]: ");
...
//				Updates.combine(Updates.set("title", title), Updates.set("wdate", wdate)));
		//static import해서 아래처럼 eq(), set()이런걸 직접 쓸 수 있다.
	...
	public void updateMany() {
		System.out.println("수정하지 않을 글의 작성자명 입력: ");
		String name=sc.nextLine();
		System.out.println("수정할 글 제목: ");
		String title=sc.nextLine();
		System.out.println("수정할 글 작성일 [yyyy-MM-dd]: ");
	...
	public void replaceOne() {
		...
		System.out.println(res.getModifiedCount()+"개의 도큐먼트가 교체됨");
	}//----------------------------------
	
	public void findOneAndReplace() {
		mcol=mdb.getCollection(table, PostVO.class);
		FindIterable<PostVO> cursor=mcol.find(eq("author", "Kim"));
